...
if "gasto" not in st.session_state:
    st.session_state.gasto = pd.DataFrame(columns=["Descripción", "Categoría", "Monto"])
...
with st.form("registro_form"):
    ...
    if submitted:
        nuevo_registro = pd.DataFrame({"Descrpcion": [descripcion], "Categoria": [categoria], "Monto": [monto]})
